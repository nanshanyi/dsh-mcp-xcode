...
//   (either may be one string or an array; patterns match the ORIGINAL Xcode
//   tool name, anchored and case-sensitive — use *List* for substring matches;
//   xcode_mcp_status is never filtered)
//
...
/**
 * Normalize an include/exclude config value: a single string, an array of
 * strings, or absent. Every character is escaped to a literal except `*`/`?`,
 * so any input compiles to a valid anchored RegExp — a pattern that matches
 * nothing is the worst case (connect() warns when filters exclude every tool).
 */
function normalizePatternList(value) {
  const raw = value === undefined ? [] : Array.isArray(value) ? value : [value]
  return raw.map((item) => wildcardToRegExp(String(item)))
}

/** Build the MCP-tool name filter from include/exclude wildcard lists. */
function buildToolFilter(includes, excludes) {
  return (toolName) => {
...
export function apply(ctx, config = {}) {
  ...
  const toolFilter = buildToolFilter(
    normalizePatternList(config.includeTools),
    normalizePatternList(config.excludeTools),
  )
  ...
  async function connect() {
    ...
    state.connectPromise = (async () => {
      try {
        ...
        state.allFiltered = filtered.length === 0 && advertised.length > 0
        if (state.allFiltered) {
          ctx.logger.warn('[%s] include/exclude filters excluded every advertised tool (%d available) — check config.includeTools / config.excludeTools', name, advertised.length)
        }
    ...
    output: {
      schema: {
        ...
        properties: {
          ...
          allFiltered: { type: 'boolean', description: 'True when include/exclude filters excluded every advertised tool.' },
        ...
        required: ['connected', 'toolCount', 'tools', 'stderrTail', 'allFiltered'],
        additionalProperties: true,
      },
      render(args, value) {
        ...
        if (value.allFiltered) lines.push('WARNING: include/exclude filters excluded every Xcode tool — check config.includeTools / config.excludeTools')
