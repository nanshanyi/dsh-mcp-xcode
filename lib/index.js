...
export function apply(ctx, config = {}) {
  ...
  ctx.effect(() => () => {
    ...
    // bridgeDisposers normally drains in onExit, but terminate() above races
    // the exit callback; clear it directly so plugin teardown never leaks.
    const bridgeList = bridgeDisposers.splice(0)
    for (const d of bridgeList) { try { d() } catch (e) {} }
  })
  ...
  function spawnBridge() {
    ...
    const onExit = (outcome) => {
      ...
      for (const entry of state.pending.values()) entry.reject(new Error('mcpbridge exited: exitCode=' + code + ' signal=' + sig))
      state.pending.clear()
      unregisterBridgeTools()
    }
    handle.done.then(onExit, onExit)
    return handle
  }

  // Unregister the bridge-generation tool registrations so a later reconnect
  // (which re-runs connect() and re-registers every tool) does not collide
  // with stale registrations from the dead bridge. The status tool lives in
  // `disposers`, not `bridgeDisposers`, so it survives bridge restarts.
  function unregisterBridgeTools() {
    const stale = bridgeDisposers.splice(0)
    for (const d of stale) { try { d() } catch (e) {} }
  }
  ...
  async function connect() {
    ...
    state.connectPromise = (async () => {
      try {
        ...
        state.tools = listed && Array.isArray(listed.tools) ? listed.tools : []
        const generation = []
        try {
          for (const t of state.tools) {
            ...
            generation.push(unregister)
          }
        } catch (e) {
          // Roll back this generation on partial-registration failure so a
          // retry does not hit "already registered".
          for (const d of generation) { try { d() } catch (err) {} }
          throw e
        }
        if (disposed) {
          for (const d of generation) { try { d() } catch (e) {} }
        } else {
          for (const d of generation) bridgeDisposers.push(d)
        }
        state.ready = true
        ctx.logger.info('[%s] connected to %s, %d Xcode MCP tools registered', name, JSON.stringify(state.serverInfo || {}), state.tools.length)
        return { connected: true, toolCount: state.tools.length }
      } catch (e) {
        state.ready = false
        throw e
      } finally {
  ...
  const statusTool = {
    ...
    description: 'Status and control of the Xcode headless MCP bridge (xcrun mcpbridge) used by the xcode_* tools. Reports connection state, server info, registered tool count/names, and the bridge stderr tail. Pass reconnect: true to kill the current bridge child, re-spawn it, and re-run the MCP handshake. If the bridge cannot connect, run "xcrun mcp-server status" / "xcrun mcp-server start" in a terminal, then call this tool with reconnect: true.',
    parameters: {
      type: 'object',
      properties: {
    ...
    async execute(args) {
      if (args && args.reconnect === true) {
        // Terminate first so onExit can observe a live handle match; then
        // force the same cleanup in case the exit callback races us.
